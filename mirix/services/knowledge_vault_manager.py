...
class KnowledgeVaultManager:
    ...
    def _apply_temporal_scoring(
        self,
        scored_memories: List[Tuple[float, KnowledgeVaultItem]],
        session: Session,
        limit: int,
    ) -> List[KnowledgeVaultItem]:
        """
    ...
    def _postgresql_fulltext_search(
        ...
        if search_field == "caption":
            ...
            rank_sql = "ts_rank_cd(to_tsvector('english', coalesce(secret_value, '')), to_tsquery('english', :tsquery), 32)"
        else:
            ...
            if query == "":
                ...
                if search_method == "embedding":
                    ...
                    if settings.mirix_pg_uri_no_default:
                        ...
                        knowledge_vault = self._apply_temporal_scoring(
                    ...
                    scored_items.sort(key=lambda x: x[0], reverse=True)
                    # Apply temporal scoring and get final results
                    knowledge_vault_items = self._apply_temporal_scoring(
                        scored_items, session, limit
                    )
