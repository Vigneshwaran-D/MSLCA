...
from mirix.settings import settings, temporal_settings
...
class EpisodicMemoryManager:
    ...
    def _apply_temporal_scoring(
        self,
        scored_memories: List[Tuple[float, EpisodicEvent]],
        session: Session,
        limit: int,
    ) -> List[EpisodicEvent]:
        """
        Apply temporal scoring to retrieved memories and handle access tracking/rehearsal.

        Args:
            scored_memories: List of (relevance_score, memory) tuples
            session: Database session for tracking access and rehearsal
            limit: Maximum number of results to return

        Returns:
            List of memories sorted by combined score
        """
        if not temporal_settings.enabled or not scored_memories:
            return [memory for score, memory in scored_memories[:limit]]

        current_time = datetime.now(dt.timezone.utc)
        combined_scored_memories = []

        for relevance_score, memory in scored_memories:
            # Track access to this memory
            temporal_service.track_access(memory, session)
            ...
            combined_score = temporal_service.combine_scores(
                relevance_score, temporal_score
            )

            # Check if rehearsal should happen
            if temporal_service.should_rehearse(memory, relevance_score):
                temporal_service.rehearse_memory(memory, session)

            combined_scored_memories.append((combined_score, memory))

        # Re-sort by combined score
        combined_scored_memories.sort(key=lambda x: x[0], reverse=True)

        # Commit all access tracking and rehearsal changes
        try:
            session.commit()
        except Exception as e:
            logger.error(f"Error committing temporal updates: {e}")
            session.rollback()

        # Return top memories
        return [memory for score, memory in combined_scored_memories[:limit]]
        ...
        with self.session_maker() as session:
            ...
            if query == "":
                ...
                base_query = select(
                    ...
                    EpisodicEvent.access_count.label("access_count"),
                    EpisodicEvent.last_accessed_at.label("last_accessed_at"),
                    EpisodicEvent.importance_score.label("importance_score"),
                    EpisodicEvent.rehearsal_count.label("rehearsal_count"),
                ).where(EpisodicEvent.user_id == actor.id)
                ...
                if search_method == "embedding":
                    ...
                    if settings.mirix_pg_uri_no_default:
                        ...
                        # Apply temporal scoring and get final results
                        episodic_memory = self._apply_temporal_scoring(
                            scored_events, session, limit
                        )
