...
class TemporalReasoningSettings(BaseSettings):
    """Settings for temporal reasoning and memory decay modeling"""
    model_config = SettingsConfigDict(env_prefix="mirix_temporal_", extra="ignore")

    # Enable/disable temporal reasoning system
    enabled: bool = True

    # Rehearsal threshold: relevance score above which memories are strengthened
    rehearsal_threshold: float = 0.7

    # Deletion threshold: temporal score below which memories are deleted
    deletion_threshold: float = 0.1

    # Exponential decay rate (λ) - higher means faster forgetting
    decay_lambda: float = 0.05

    # Power law decay exponent (α) - controls long-term retention
    decay_alpha: float = 1.5

    # Maximum age in days before hard delete regardless of score
    max_age_days: int = 365

    # Weight for relevance score (BM25/embedding) in final scoring
    retrieval_weight_relevance: float = 0.6

    # Weight for temporal score in final scoring
    retrieval_weight_temporal: float = 0.4

    # Importance boost per rehearsal (added to importance_score)
...
class ModelSettings(BaseSettings):
    ...
    # Bedrock
    # Supports both AWS_ACCESS_KEY and AWS_ACCESS_KEY_ID
    ...
    # AWS Bedrock Inference Profile ARN (required for on-demand throughput)
    # Example: arn:aws:bedrock:us-east-1:123456789012:inference-profile/us.anthropic.claude-opus-4-1-20250805-v1:0
    aws_bedrock_model_arn: Optional[str] = Field(
        None,
        validation_alias=AliasChoices('aws_bedrock_model_arn', 'AWS_BEDROCK_MODEL_ARN')
    )
