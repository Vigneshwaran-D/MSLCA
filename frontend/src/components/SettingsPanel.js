...
const SettingsPanel = ({ settings, onSettingsChange, onApiKeyCheck, onApiKeyRequired, isVisible }) => {
  ...
  const [modelProviderFilter, setModelProviderFilter] = useState('all'); // 'all', 'gemini', 'bedrock', 'openai', 'anthropic'
  ...
  const baseModels = [
    ...
    ...modelsByProvider.bedrock
  ];

  // Filter models based on provider selection
  const getFilteredModels = (providerFilter) => {
    if (providerFilter === 'all') {
      return [...baseModels, ...customModels];
    }
    return [...(modelsByProvider[providerFilter] || []), ...customModels];
  };

  // Combine base models with custom models based on filter
  const models = getFilteredModels(modelProviderFilter);
  ...
  // Helper function to get friendly model name with provider badge
  const getModelDisplayName = (modelName) => {
    // Check if it's a Bedrock model (starts with 'anthropic.')
    if (modelName.startsWith('anthropic.')) {
      const shortName = modelName.replace('anthropic.', '').replace(/-v\d+:\d+$/, '');
      return `🔷 ${shortName} (Bedrock)`;
    }
      ...
      <div className="settings-content">
        <div className="settings-section">
          ...
          <div className="setting-item">
            <label htmlFor="provider-filter">{t('settings.modelProvider', 'Model Provider')}</label>
            <select
              id="provider-filter"
            ...
            >
              <option value="all">{t('settings.providers.all', 'All Providers')}</option>
              <option value="gemini">{t('settings.providers.gemini', 'Google Gemini')}</option>
              <option value="bedrock">{t('settings.providers.bedrock', 'AWS Bedrock')}</option>
              <option value="openai">{t('settings.providers.openai', 'OpenAI')}</option>
              <option value="anthropic">{t('settings.providers.anthropic', 'Anthropic')}</option>
            </select>
            <span className="setting-description">
              {t('settings.descriptions.modelProvider', 'Filter available models by provider. Gemini is the default, but you can switch to AWS Bedrock or other providers.')}
            </span>
          </div>
